 ...
 */
export function pruneEchoBacklog(db: Database.Database | PrunableMailbox, opts: { now?: number } = {}): number {
  const keep = ECHO_BACKLOG_CAP;
  const cutoffIso = new Date((opts.now ?? Date.now()) - ECHO_MAX_AGE_DAYS * 24 * 60 * 60 * 1000).toISOString();

  if ('prunePendingMessages' in db) {
    return db.prunePendingMessages(ECHO_CHANNEL_TYPE, cutoffIso, keep);
  }

...
- **Agents now actually receive their capability instructions.** Each group's `CLAUDE.md` was a list of `@` imports pointing at symlinks into `/app`, and Claude Code silently drops any import that resolves outside the project directory — so the runtime contract, all six module manuals (`ncl`, scheduling, self-modification, sub-agents, interactive prompts, outbound tools) and the credential-gateway prose never reached the model. The project document is now one flat file with every source inlined. No permission changes, but agents will start attempting capabilities they previously did not know existed, each of which routes through an approval gate that has barely been exercised.
- **One composer for every provider.** `composeProjectDoc` on trunk now builds the project document for Claude and (via `/add-codex`) Codex from one spec, closing two silent divergences: a Codex group with `cli_scope: disabled` was still handed the scheduling manual for commands host dispatch rejects, and Codex never received resident skill prose at all.
- **The Claude project document is now capped at the size Claude Code will still load.** Claude Code loads a `CLAUDE.md` of up to 4 MiB and skips a larger file outright, so an unbounded document means an agent with no instructions at all and no error. Over the cap the composer drops its largest optional sections, says so in the document, and logs at error level.
- **Resident skill prose now follows the group's skill selection.** A skill's always-on `instructions.md` is inlined only when that skill is in the group's `container.json` selection, matching the skill links the runner plants. Previously the composer inlined every shipping skill's prose regardless, so a narrowed group could be taught rules for a skill whose `SKILL.md` it did not have.
- **The composer's temp file can no longer be redirected by the agent.** The group folder is the agent's read-write working directory, so the old `CLAUDE.md.tmp-<pid>` name was a path the agent could pre-create: a symlink there redirected the host's write to any file the host user could reach, and a directory there failed every spawn. The temp name is now random and opened with `O_EXCL`.
- **Upgraded installs are cleaned up automatically.** The first host start after this release removes the leftover `.claude-fragments/` directory and `.claude-shared.md` symlink from every group folder.

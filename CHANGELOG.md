...
- **Remote MCP servers can use HTTPS Streamable HTTP.** Register them with `ncl groups config add-mcp-server --name <name> --url <https-url>` or the existing admin-approved `add_mcp_server` tool. Local stdio MCP commands keep their current `command` / `args` / `env` behavior; remote credentials remain OneCLI-managed, and URLs with userinfo, query parameters, or fragments are rejected.

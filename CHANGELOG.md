...
- **Agents now receive the capability instructions they were always meant to have.** Each group's `CLAUDE.md` was a list of `@` imports pointing at symlinks into `/app`, and Claude Code silently drops any import resolving outside the project directory, so the runtime contract, all six module manuals and the credential-gateway rules never reached the model. It is now one flat file with every source inlined, composed by one `composeProjectDoc` shared with the Codex provider. Side effects: the document is capped at the 4 MiB Claude Code will still load; resident skill prose follows the group's skill selection; stored MCP servers are re-validated before their instructions are inlined; a Codex group with `cli_scope: disabled` no longer gets the `ncl tasks` manual. **No operator action** — the first host start after upgrading removes the leftover `.claude-fragments/` and `.claude-shared.md` from every group folder. See [docs/SECURITY.md](docs/SECURITY.md#2-mount-security) for the resulting mount set.
